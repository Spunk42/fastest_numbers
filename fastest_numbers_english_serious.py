...
# each number has a normal name, ordinal name, and multiple name
# the multiple name is used to handle 'sixes' and 'twenties' instead of sixs and twentys. the s is applied later
one_names = [
    [["nought", 1], ["", 0], ["", 0]],
    [["one", 1], ["first", 1], ["one", 1]],
...
# the very large names are used for names such as "floor cube root seven quintillion"
# we don't have ordinal since there is no use case
large_names = [
    ["hundred",2,100,2,"hundredth",2],
    ["thousand",2,1000,3,"thousandth",2],
    ["million",2,1000000,6,"millionth",2],
    ["billion",2,1000000000,9,"billionth",2],
    ...
    ["duodecillion",5,1,39],
    ["tredecillion",4,1,42],
    ["quattuordecillion",5,1,45],
    ["quindecillion",4,1,48],
    ["sexdecillion",4,1,51],
    ["septendecillion",5,1,54],
    ["octodecillion",5,1,57],
    ["novemdecillion",5,1,60],
    ["vigintillion",4,1,63],
]
...
def base_syllables(n):
    if n < 20:
        ...
        n_div_name = number_names[n_div]["names"][1] + " " if n_div != 1 else "a " # a hundred instead of one hundred
...
# process multiples and fractions such as "thirties" and "sevenths"
def unary_name(u,n):
    if u == 1 and n == 2:
        return 1, "halve"
    if n < 20:
...
def number_names_generator(leave_point,max_number):
    ...
    u_types = [0,["nths"," / "]]#,["ns"," * "]] # removed multiples in serious version
    for x in range(99,1,-1):
        for u in range(1,len(u_types)):
    ...
    sub_units = [["hundred",100,2],["lakh",100000,1],["crore",10000000,1]]
    for u in sub_units:
    ...
    min_missing = 1
    max_log = math.log(max_number)-0.001
                ...
                for huge in huge_numbers[remaining_syllables]:
                    ...
                    for u in range(4,pemdas_count):
                        if number_names[op_output]["syllables"][u] >= s:
                            number_names[op_output]["names"][u] = new_name
                            number_names[op_output]["equations"][u] = new_equation
                            number_names[op_output]["level_out"][u] = 4
...
unary = [
    ...
    { "id": "adj_root", "syllables": 1, "adjust_type": "round", "pemdas_left": 2,"pemdas_result": 3, "level_in": 0, "level_out": 4},
    { "id": "adj_root", "syllables": 1, "adjust_type": "floor", "pemdas_left": 2,"pemdas_result": 3, "level_in": 0, "level_out": 4},
    { "id": "adj_root", "syllables": 2, "adjust_type": "ceiling", "pemdas_left": 2,"pemdas_result": 3, "level_in": 0, "level_out": 4},
...
for i in irrationals:
    for a in adjust_types:
        unary.append({
            ...
            "equation": i["equation"],
            "value": i["value"], 
            "pemdas_left": 1, "pemdas_result": 3, "level_in": 0, "level_out": 4
        })
...
binary = [
    ...
    { "id": "adj_fraction", "syllables": 1, "adjust_type": "floor", "text": " ", "pemdas_left": 2,"pemdas_right": 0,"pemdas_result": 3, "level_in": 0, "level_out": 4},
    { "id": "adj_fraction", "syllables": 2, "adjust_type": "ceiling", "text": " ", "pemdas_left": 2,"pemdas_right": 0,"pemdas_result": 3, "level_in": 0, "level_out": 4},
...
numbers_out(eff_names, 'fastest_numbers_serious_dev.csv')
